...
- **Photo pop-ups** — the "Photos" section (`#gallery`) has a pool of 10 placeholder slots in `script.js` (`photoPool`). Each page load randomly shows 5-6 of them, staggered in like they're being pinned up. Drop real photos in an `images/` folder, update the `src` paths in that array, then swap the placeholder `<div class="photo-pop-frame">` in `renderPhotoBoard()` for a real `<img src="${photo.src}" alt="${photo.caption}">`.
